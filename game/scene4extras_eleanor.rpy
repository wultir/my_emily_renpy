label day6_eleanor_home:
    scene bg 0654 with Dissolve(1.0)

    mc "I SAID GO TO BED!!!!"


label scene0655:
    scene bg 0655 with Dissolve(1.0)
    "?" "Huh??"

    mc "FUCK!"


label scene0655a:
    scene bg 0655a with Dissolve(1.0)

    ele "Oh!"


label scene0655b:
    scene bg 0655b with Dissolve(1.0)

    ele "Ah hah"
    ele "Uh huh"
    ele "Yes he is!"
    ele "I will!"
    ele "Oh yes he is hihi"
    ele "Umm.. Ok"
    ele "Love you! Kisses!"


label scene0656:
    scene bg 0656 with Dissolve(1.0)

    ele "That was Em on the phone, she sent kisses to you"
    ele "And she told me to make sure you wear every piece she put on the dresser behind"
    ele "Oh and Hiiiiya!"
    ele "I'm alone now and I know you are too so I thought we should go and get a coffee!"

    mc "Hey.. I... I.. You? Now?"
    mc "I mean go? Now?"
    mc "If that.. ok, sure. I.."
    mc "What?"
    mc "Fuck"


label scene0657:
    scene bg 0657 with Dissolve(1.0)
#Elanor
    ele "Sorry!"
    ele "I didn't think you would march in that naked.."
    ele "Haha"


label scene0658:
    scene bg 0658 with Dissolve(1.0)

    ele "Let's see what she chose for you!"

    mc "Oh no fuck me! I see pink!"

    ele "Excuse me!"

label scene0659:
    scene bg 0659 with Dissolve(1.0)

    ele "What are these? "
    ele "Panties?"

    mc "..."
    mc "Fuck"
    mc "FUCK FUCK FUCK FUCK FUCK"
    mc "PINK FUCKING PANTIES"
    mc "FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK FUCK"


label scene0660:
    scene bg 0660 with Dissolve(1.0)

    ele "Ohhhhh "
    ele "Jockstraps!"

    mc "What are they??"
    mc "Aren't they like panties for gays??"

    ele "What? "
    ele "Hahahaha omg "
    ele "No!"
    ele "Well if you put it like that.. Maybe sometimes?"

    mc "But they aren't panties right?"

    ele "No!"

    mc "They aren't for females?"

    ele "Haha NO!"
    ele "I like them! They are sexy!"
    ele "Stop holding your cock hihi and put these on!"


label scene0661:
    scene bg 0661 with Dissolve(1.0)
    "When realising you are still naked and holding your cock in your hand you grab the straps and jump into them"

    ele "Sexy!"
    ele "I told you so, now turn around!"


label scene0662:
    scene bg 0662 with Dissolve(1.0)
    "Defeated and humiliated you turn around"

    ele "Hey don't look so gloomy! They are sexy!"
    ele "They look perfect on your butt.. I'll have to buy a couple of those!"

    "Suddenly the tights and your pink t-shirt doesn't bother you anymore. And you get dressed really fast under Elanors gaze"


label scene0663:
    scene bg 0663 with Dissolve(1.0)

    mc "Nice!"


label scene0664:
    scene bg 0664 with Dissolve(1.0)

    ele "You like it?"

    mc "Yeah.. Looks expensive!"


label scene0665:
    scene bg 0665 with Dissolve(1.0)

    ele "Oh I'm sure it was"
    ele "It's important for some men to have blonde wifes with big breasts and stupid cars like this.."
    ele "But I don't complain! I get access to some nice things too hihi"


label scene0666:
    scene bg 0666 with Dissolve(1.0)

    mc "Oh like what?"
    mc "Now I'm curious!"


label scene0667:
    scene bg 0667 with Dissolve(1.0)

    ele "Curiosity killed the cat!"
    ele "I get access to stuff like credit cards and other.. secrets.."
    ele "Never ask a girl about her secrets hihi"

    "When she presses the start button it sounds like a war broke out right behind you and like the world is going under"


label scene0668:
    scene bg 0668 with Dissolve(1.0)

    mc "Ohhh myyy goddddd!"

    ele "Yeah I know! It's so violent.. But I'll drive slowly. This car scares me!"


label scene0669:
    scene bg 0669 with Dissolve(1.0)

    mc "Where are we going?"

    ele "I know a good place, right in the city. Just a couple of minutes away!"


label scene0670:
    scene bg 0670 with Dissolve(1.0)
    "You arrive and you take a seat while Elanor goes and orders something"


label scene0671:
    scene bg 0671 with Dissolve(1.0)

    ele "Here you go!"
    ele "Have you ever tasted macarons, they are like orgasms on clouds!"
    ele "And I know you aren't a fan of coffee so I got you this. It's called Milkers tears."


label scene0672:
    scene bg 0672 with Dissolve(1.0)

    ele "And a big steaming hot black one for me.. I love them black haha"
    ele "So how's life treating you?"
    ele "Tell me everything that's new!"


label scene0673:
    scene bg black with Dissolve(1.0)

    "After 15 minutes or so talking about life in general"


    scene bg 0673 with Dissolve(1.0)

    ele "Forgive me for asking, but we are family now and you guys are young.."
    ele "So how's your sex? Is it good?"

    mc "Whaa?.. "
    mc "Its.."

    menu:
        "We are trying some bdsm.. I think.. [pink]":
            jump day6_bdsm
        "It's ok... [red]":
            jump day6_its_ok


label day6_bdsm:

    ele "And you are the sub right?"

    mc "How can you know that? What if I'm not?"

    ele "Haha.. I knew it!"
    ele "Relax, it's not a bad thing!"
    ele "I know my baby and she's just like me.. We like softer men. "
    ele "We like control and you are just better as life partners.. Don't take me wrong, we can get beaten into total submission too.. But it takes a really special man for that. But we wouldn't be able to live like that forever. We need power and control.."
    ele "And I know she loves you. I see it in her eyes and hear it on her voice when we talk about you or love in general. We are also often on the phone when you are out somewhere and she's always worried sick. We both have trouble opening up and expressing our love for the ones we love. But trust me, she loves you.."
    ele "So stop worrying, you are still young and have so many new things to explore together. It's gonna be a wild but pleasant ride honey!"
    ele "I've seen and tried most of it, so you can ask me anything if you ever get stuck or need advice on anything at all"

    mc "Thank you!"

    jump day6_sex_question_done


label day6_its_ok:

    ele "Oh.. Well.. Ok.."
    ele "If you ever need help or answers you can ask me, I've seen most of it.."

    mc "Thank you"


label day6_sex_question_done:
    "ring ring*"

    show phone mc_em_call with Dissolve(1.0):
        pos (510, 2)
        anchor(0, 0)

    mc "Hello"

    em "Hey babe"
    em "Are you with my mom now?"

    mc "Yeah!"

    em "Sorry I forgot to tell you about her coming over "
    em "You kept my mind elsewhere haha!"
    em "Anyways.. You didn't receive any challenges from mistress Victoria right?"

    mc "No I didn't... Did you?"

    em "Yes!"
    em "It's hair day! Yaaay!"
    em "I already got my hair done.. I'm at the office today, James isn't here and we have so little work today so I treated myself"
    em "I booked you a time. You have to be there in 1 hour and 30 minutes!"
    em "Don't forget!"
    em "I'll text you the place and where it is"
    em "I won't be there because I'm running to the office again just for some small paper work. But I'll come over there when they are doing you!"
    em "I have to go now"
    em "See you soon "
    em "1 hour, 30 minutes!"
    em "Love you"
    em "Bye!"

    mc "Bye!"

    hide phone with Dissolve(1.0)


label scene0674:
    scene bg 0674 with Dissolve(1.0)

    ele "Was it Em?"

    mc "Yeah!"
    mc "i'm apparently getting my hair done in 1 hour and 30 minutes"

    ele "Haha yup that's my girl"
    ele "By the way I meant to tell you this earlier but your ding dong shocked me hahaha"
    ele "You look lovely clean shaven like that! It suits you much better!"

    mc "Thank you... And sorry!"
    mc "It's so embarrassing!"

    ele "Don't be.. We are family right!"
    ele "You saw my breasts so its only fair!"

    mc "Haha.. Yeah but I didn't see your... ding dong..."

    ele "Chuckles*"
    ele "Honey It would shock me more. There's shouldn't be any ding dong inside my panties!"

    mc "Haha"
    mc "What do you have then?"

    ele "Hmm"
    ele "Maybe a dong only?"
    ele "Ding?"
    ele "Bong?"
    ele "They all sound like cocks"
    ele "I just have a pussy!"
    ele "Haha"

    mc "Haha okey, can't wait to see it then!"

    ele "Hahaha omg you naughty boy!"
    ele "Is that how you talk to your mommy?"
    ele "Hahaha"

    scene bg black with Dissolve(1.0)

    "You both keep talking for an hour so. You feel relaxed around her and you both keep laughing alot"
    "Elanor has a meeting and so do you so you both decide to leave"

    jump day6_eleanor_done
